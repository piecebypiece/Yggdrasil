...
namespace TestExcelDataReader
{
	...
	public class ExcelReader
	{
		...
		//public bool _MakeExcelDataToCSFileAndTXTFile(string path)
		//{
		...
		{
			Microsoft.Office.Interop.Excel.Application m_app = new Microsoft.Office.Interop.Excel.Application();
			m_workbook = m_app.Workbooks.Open(path);

			//추가(시트 이름 시트 수만큼 가져오기)
			m_SheetsName = new string[m_workbook.Worksheets.Count]; //엑셀 시트 갯수만큼 체크.
			int i = 0;
			foreach (Microsoft.Office.Interop.Excel.Worksheet wSheet in m_workbook.Worksheets)   //sheetsName배열에 시트 갯수만큼 이름넣는 작업.
			{
				m_SheetsName[i] = wSheet.Name;
				i++;
			}
			...
			//String[] excelSheets = new String[excelBook.Worksheets.Count];  //엑셀 시트 갯수 만큼 체크.
			//int i = 0;
		...
		// 추가 한것
		public void ReadSheet()  //오버로딩 : 시트가 여러개 일때 작업.
		{
			for (int i = 0; i < m_SheetsName.Length; i++)
			{
				ReadSheet(m_SheetsName[i]);
			}
		}
		...
		void WriteFile(Excel.Worksheet sheet, int left, int right, int top, int bottom)
		{
			...
			//D:\GitSources\Yggdrasil\ExcelData\Data_Table\BossSkill.xlsx
			//idx => D:\GitSource\Yggdrasil\Assets\00.Data\Excel\asd.txt
			

			string temppath = filepath.Substring(0, idx);
			int idx2 = temppath.LastIndexOf("\\");
			string temppath2 = temppath.Substring(0, idx2);
			// temppath2 ->  D:\GitSource\Yggdrasil\Assets\00.Data 이 경로까지만 뽑아오고 싶어서 (lastIndexof가 두번되는 위치)

			// 기본경로(파일경로 안에 save경로 설정.)
			...
			// 혹시 파일을 넣은 상위폴더에 txt폴더나 script폴더가 있으면 해당 폴더로 save경로 지정. 
			DirectoryInfo di = new DirectoryInfo(temppath2);
			DirectoryInfo[] diarr = di.GetDirectories();

			//해당 경로에 script폴더와 txt폴더가 있는지 확인 있을경우 save경로를 다시 설정.
			foreach (DirectoryInfo element in diarr)
			{
				...
				if (f_name == "Txt")
				{
					savepath_txt = System.IO.Path.Combine(temppath2, "Txt", sheetName + ".txt");
				}
				else if (f_name == "Script")
				{
					savepath_cs = System.IO.Path.Combine(temppath2, "Script", structName + "Loader" + ".cs");
				}
			}

			int idx3 = savepath_txt.IndexOf("Assets", 0, savepath_txt.Length);
			string createpath = savepath_txt.Substring(idx3);
			...
			sb.Append(tab + "[SerializeField] string filepath =" +"@" + "\"" + createpath + "\"" + ";\n");

			//sb.Append(savepath_txt + ";\n");
